...
usage="\nUsage: %s --train SDF1 [SDF2 SDF3 ...] [--test SDF1 [SDF2 SDF3 ...]]\n\n"
...
for sdf in "$@"; do
  case $sdf in
    ...
    *)
      ...
      if [ $train -eq 1 ]; then
        train_files+=("$cm_filename")
      else
        test_files+=("$cm_filename")
      fi
      echo
      echo "Cleaning, filtering and extracting all monoprotic structures ($(basename "$sdf"))..."
      python scripts/gen_clean_mono_dataset.py "$sdf" "$cm_filename" -kp pKa || exit 2
    ;;
  esac
done
...
python scripts/combine_datasets.py "${train_files[@]}" || exit 3
